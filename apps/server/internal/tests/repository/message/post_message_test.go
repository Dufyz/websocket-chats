...
func TestRepositoryPostMessage(t *testing.T) {
	...
	assert.Equal(t, postMessage.Chat_id, message.Chat_id)
	assert.Equal(t, postMessage.User_id, message.User_id)
	assert.Equal(t, postMessage.Message, message.Message)
}
